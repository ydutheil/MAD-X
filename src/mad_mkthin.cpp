/* mad_mkthin.cpp
 ...
 2018 : Thick solenoid slicing, write bend angle to multipole if different from k0*l, new elements from element definition - all attributes enabled
...
public:
  ...
  ~SliceDistPos()=default; // destructor
  void Print(std::ostream &StrOut = std::cout) const;
  double delta;
  double Delta;
  //
...
public:
  ...
  void  current_node(node* thisnode) { thick_node=thisnode; } // set
private:
  ...
  ElementListWithSlices *theSliceList, *theRbendList, *theBendEdgeList; // Elements, of various types; consider to make separate lists for quadrupoles, sext ..  to speed up search
  sequence *sliced_seq;
...
{
  if(MaTh::Verbose>1) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << " el=" << el << std::endl;
  if(el)
  {
    const command* el_def=el->def;
    name_list* nl=el_def->par_names;
...
static std::string dump_slices() // Loops over all current elements and prints the number of slices. Used for debug and info, uses global element_list
{
  std::ostringstream ostr;
  ostr << "++++++ dump_slices";
  if(MaTh::Verbose>1)
  ...
  {
    element* el = element_list->elem[i];
    const command* el_def=el->def;
    name_list* nl=el_def->par_names;
  ...
  ostr << "------ end of dump slices. There were " << std::setw(4) << element_list->curr << " elements, "
...
static int get_slices_from_elem(const element* el)
{
  ...
  {
    inf=nl->inform[ei];
    if(inf) slices=el->def->par->parameters[ei]->double_value;
  }
  if(MaTh::Verbose>1) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__  << " element " << el->name << " ei=" << ei << " inf=" << inf << " slices=" << slices << '\n';
  return slices;
}
...
static void add_half_angle_to(const element* rbend_el,element* to_el,const std::string to_parm) // get half surface angle of rbend, add to e1 or e2 of dipedge or sbend
{
  if(rbend_el && to_el)
  {
    expression* half_angle_expr  = compound_expr(my_get_param_expression(rbend_el,"angle"),0.,"/",nullptr,2); // angle/0.5
    command_parameter* to_param = return_param_recurse(to_parm.c_str(),to_el); // get param from element, use here the non const version of return_param_recurse, to modify to_el
    if(to_param) // modify the existing parameter in to_el
    {
      if(MaTh::Verbose>1) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << " to_parm=" << to_parm << "    original to_param " << my_dump_command_parameter(to_param);
      to_param->expr = compound_expr(to_param->expr,0,"+",half_angle_expr,0);
      if(MaTh::Verbose>1) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << " to_parm=" << to_parm << "        now  to_param " << my_dump_command_parameter(to_param);
    }
    else // param was null in to_el, start from parameter definition
    {
      int ipar = name_list_pos(to_parm.c_str(),to_el->def->par_names);
      ...
      if(MaTh::Verbose>1) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << " to_parm=" << to_parm << "  was 0, now  to_param " << my_dump_command_parameter(to_param);
    }
  }
}
...
{
  if(MaTh::Verbose>1) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << " slice_style=\"" << slice_style << "\"" << '\n';
  
  sequence* sliced_seq;
  static SequenceList sliced_seqlist; // Warning: memory deleted only on exit...
  
  if(slice_style==std::string("AllDone")) { sliced_seqlist.Reset(); return nullptr; } // clear the list when all (sequence and subsequences) done. Allows to call makethin again
  
  if((sliced_seq=sliced_seqlist.get_sequ(thick_sequ)))
  {
    if(MaTh::Verbose) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << " sequence " << thick_sequ->name << " was already sliced" << '\n';
    if(MaTh::Verbose>1) std::cout << sliced_seqlist;
  ...
  SeqElList theSeqElList(name, slice_style, sliced_seq, thick_sequ->start);
  while(theSeqElList.current_node() != nullptr) // in current sequence, loop over nodes
  {
    if(MaTh::Verbose) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << " " << theSeqElList.current_node()->name << '\n';
    theSeqElList.slice_node(); // decides what to do with current node :  slice_node_translate, slice_node_default or nothing
    if (theSeqElList.current_node() == thick_sequ->end)  break;
    theSeqElList.current_node(theSeqElList.current_node()->next);
  }
  sliced_seq->end->next = sliced_seq->start;
  
  if(MaTh::Verbose>1)  std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << '\n' << '\n' << '\n' << " my_dump_sequence sliced_seq " << my_dump_sequence(sliced_seq,2)  << '\n'; // dump level 2, without nodes/elements
  ...
  if(MaTh::Verbose) std::cout << theSeqElList; // print final list
  return sliced_seq;
} // slice_sequence
...
{ // copy all active attributes from elem_inp to cmd if they exist in cmd; used to create new thin or modified thick elements
  ...
  if(MaTh::Verbose>1) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << " elem_inp->name=" << elem_inp->name << '\n' << theElmAttr;
  if(MaTh::Verbose>1) std::cout << elem_inp->name << ":" << '\n' << my_dump_element(elem_inp) << '\n'; // CSPE
...
static element* new_marker_element(const std::string el_name, const element* el_inp)
{
  command* cmd = new_cmdptr( find_element("marker", base_type_list) );
  copy_params_from_elem(cmd,el_inp,MaTh::DoNotCopy);
  if(MaTh::Verbose>1) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << " el_out_cmd " << my_dump_command(cmd) << std::endl;
  return make_element(el_name.c_str(),"marker",cmd,-1);
}
...
static int set_selected_elements() //  result in global  element_list     used in dump_slices, force_consistent_slices
{
  ...
  if (MaTh::Verbose) std::cout << dump_slices(); // shows where 2 or more slices were selected
  return 0;
}
...
void makethin(in_cmd* cmd) // public interface to slice sequence, called by exec_command from mad_cmd.c
{
  ...
  if (slice_select->curr > 0)
  {
    ...
    if (MaTh::Verbose&& iret) std::cout << "set_selected_elements iret=" << iret << '\n';
  }
  ...
  if (nl->inform[ipos_seq] && (name = pl->parameters[ipos_seq]->string))
  {
    ...
    if (ipos2 >= 0)
    {
      ...
      if (MaTh::Verbose) std::cout << "makethin: sliced_seq->start->name '" << sliced_seq->start->name << "', sliced_seq->end->name '" << sliced_seq->end->name << "'" << '\n';
      if (MaTh::Verbose) std::cout << "makethin: sliced_seq->start->previous->name '" << sliced_seq->start->previous->name << "', sliced_seq->end->next->name '" << sliced_seq->end->next->name << "'" << '\n';
    }
    else warning("unknown sequence ignored:", name);
  }
  else warning("makethin without sequence:", "ignored");
  slice_sequence("AllDone",nullptr); // signal that this sequence including subsequences was done. Clear list - to allow to slice again.
...
OneElementWithSlices::OneElementWithSlices(const element* thick_elem,element* sliced_elem) : thick_elem(thick_elem) // OneElementWithSlices constructor
{
  ...
  if(MaTh::Verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " OneElementWithSlices constructor called  thick_elem->name=" <<  thick_elem->name << '\n';
}
...
{
  if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " ElementListWithSlices destructor called VecElemWithSlices.size()=" << VecElemWithSlices.size() << '\n';
...
int ElementListWithSlices::find_thick(const element* thick_elem) // find thick_element pointer in VecElemWithSlices
{
  ...
  {
    if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " VecElemWithSlices.size()=" << std::setw(4) << VecElemWithSlices.size()
      << " " << std::left << std::setw(MaTh::el_name_maxlen) << thick_elem->name << std::right;
...
element* ElementListWithSlices::find_slice(const element* thick_elem,const std::string name) // find address of thin slice by slice name for thick_elem
{
  ...
  if(iel<0)
  {
    if(verbose>1) std::cout << " find_slice=" << std::left << std::setw(MaTh::el_name_maxlen) << name << " thick_elem=" << std::setw(MaTh::el_name_maxlen) << thick_elem->name << std::right << " not (yet) known" << '\n';
    return nullptr;
  }
  element* result=nullptr;
  ...
  if(verbose>1 && result==nullptr) std::cout << " find_slice returns NULL for " << name << '\n';
  return result;
}
...
{
  if(verbose>1) StrOut << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " VecElemWithSlices.size()=" << VecElemWithSlices.size() << '\n';
...
: thick_node(thick_node), sliced_seq(sliced_seq), seqname(seqname), slice_style(slice_style), verbose(MaTh::Verbose), eps(1.e-15), MakeDipedge(MaTh::iMakeDipedge) // SeqElList constructor, eps used to check if values are is compatible with zero
{
  theSliceList    = new ElementListWithSlices(verbose);
  theRbendList    = new ElementListWithSlices(verbose);
  theBendEdgeList = new ElementListWithSlices(verbose);
  if(verbose && !MaTh::iMakeDipedge) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " ***  makedipedge is off, should always be on except for backwards compatibility checks  *** " << '\n';
  if(verbose > 1)              std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " constructor seqname=" << seqname << " makedipedge check  MakeDipedge=" << MaTh::iMakeDipedge << '\n';
}
...
{ // read k0-k3, k0s-k3s in thick_elem and put them in kn_pars, ks_pars;  clone to allow for changes in copy
  if(verbose>1)
  {
    std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__;
    std::cout << " kn_pars=";
...
command_parameter* SeqElList::make_k_list(const std::string parnam,command_parameter* k_pars[4],command_parameter* k_param) const
...
element* SeqElList::sbend_from_rbend(const element* rbend_el)
{
  ...
  const std::string sbend_name=rbend_name; // optionally add to name +"_s";
  element* sbend_el = theRbendList->find_slice(rbend_el,sbend_name);
  
  if(sbend_el) return sbend_el; // was already done
  
  command* sbend_cmd = new_cmdptr( find_element("sbend", base_type_list) );
  copy_params_from_elem(sbend_cmd,rbend_el,{"slot_id","assembly_id"});
  
  // --   at this point, the rbend length should already be in sbend_cmd, but needs to be modified when rbarc is on (default)
  if( verbose>1 ) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " sbend_cmd:" << '\n' << my_dump_command(sbend_cmd);
  if(rbarc_fl())
  {
  ...
  theRbendList->put_slice(rbend_el,sbend_el); // for information, keep address of translated rbend_el
...
element* SeqElList::create_thick_slice(const element* thick_elem,const int slice_type) // create entry/body/exit slice elements
{
  ...
  if (verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ <<  my_dump_element(sliced_elem) << '\n';
...
element* SeqElList::create_sliced_magnet(const element* thick_elem, int slice_no,bool ThickSLice) // create the sliced element, recursively for parents
{
  ...
  else sliced_elem_parent = create_sliced_magnet(thick_elem->parent,slice_no,ThickSLice); // recursively slice parent       --- element specific
  ...
  { // from  knl:={amb ,( kmb ) * ( lmb ) };   to
    if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " ThickSLice=" << ThickSLice << " set kn, ks to zero" << '\n';
  }
  ...
  SetParameter_in_cmd(cmd,kn_param,"knl",1);
  SetParameter_in_cmd(cmd,ks_param,"ksl",1);
...
element* SeqElList::create_thin_solenoid(const element* thick_elem, int slice_no) // create the sliced element, recursively for parents
{
  ...
  command_parameter* ksi_par=par_scaled(ks_param,length_param,"ksi",nslices);
  command* cmd = new_cmdptr( thick_elem );
...
void SeqElList::slice_node_translate() // slice/translate and add slices to sliced sequence  sliced_seq
{
  ...
  element* thick_elem=thick_node->p_elem; // work directly on this element  --  to do translation only once, element maybe used several times in sequence
  
  if( std::string(thick_node->base_name)=="rbend" && nslices>0 ) // rbend translation to sbend
  {
    ...
    strcpy(thick_node->base_name,"sbend");
    if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " thick_node->base_name=" << thick_node->base_name << '\n';
  ...
  {
    if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " " << thick_elem->name << " ThickSLice=" << ThickSLice << " nslices=" << nslices << " place thick_node " << thick_node->name << " without slicing" << '\n';
  ...
  if(verbose>1)
  {
    std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " " << thick_elem->name << " " << thick_node->base_name << " slice_style=\"" << slice_style << "\""
  ...
  element *EntryDipedge=nullptr, *ExitDipedge=nullptr, *sbend_el=nullptr;
  ...
  if( IsBend && MakeDipedge) // find any existing EntryDipedge, sbend_el, ExitDipedge    and use them
  {
    ...
    if(verbose>1)
    {
      if(verbose>1)    std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__  << "              " << std::setw(MaTh::par_name_maxlen) <<   thick_elem->name << " " << thick_node->base_name << '\n';
      ...
      if(sbend_el)     std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__  << "     sbend_el=" << std::setw(MaTh::par_name_maxlen) <<     sbend_el->name << " already exists " << EntryDipedge << '\n';
    }
  }
  
  if( IsBend && MakeDipedge && EntryDipedge==nullptr) // create new EntryDipedge for this bend
  { // first look if e1 or h1 are there
    ...
    if (command_par_value("kill_ent_fringe",thick_elem->def) == false)
    {
      ...
      if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__  << " now EntryDipedge=" << EntryDipedge << '\n';
    }
  }
  
  if( IsBend && MakeDipedge && ExitDipedge==nullptr) // create new ExitDipedge for this bend
  {
    if (command_par_value("kill_exi_fringe",thick_elem->def) == false)
    {
      ...
      if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__  << " now  ExitDipedge=" << EntryDipedge << " " << my_dump_element(ExitDipedge) << '\n';
    }
  } // new ExitDipedge
  
  if(IsBend && MakeDipedge) Remove_All_Fringe_Field_Parameters(thick_elem); // remove what is taken care of by dipedges
  
  // prepare for slicing
  element *sliced_elem=nullptr; // pointer to new sliced element
  ...
  {
    if(strstr(thick_node->base_name,"elseparator")) sliced_elem = create_thin_elseparator(thick_elem,1); // create the first thin elseparator slice
    else if (IsSolenoid && !ThickSLice)             sliced_elem = create_thin_solenoid(thick_elem,1);    // create the initial solenoid slice
    else                                            sliced_elem = create_sliced_magnet(thick_elem,1,ThickSLice); // create the initial magnet slice
    if(verbose>1) if(sliced_elem) std::cout << sliced_elem->name << " nslices=" << nslices << " sliced_elem now " << my_dump_element(sliced_elem); // collimator lrad here and active
  }
  
  if(verbose>1)
  {
    std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ <<  " thick " << thick_elem->name  << " of type " << thick_node->base_name << " done with element slice generation ";
  ...
  if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " length_param=" << length_param << " l_expr=" << l_expr << '\n';
  ...
  {
    if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " nslices=" << nslices << " i=" << i << '\n';
    ...
    {
      if (strstr(thick_node->base_name,"elseparator")) slice_i = create_thin_elseparator(thick_elem,i);
      ...
      {
        if( MaTh::iMoreExpressions<1 ) thin_at_expr = compound_expr(at_expr,thick_node->at_value,"+",  nullptr, length*at_shift(nslices,i,local_slice_style) );  // use length and shift values, no expressions
    ...
    if(verbose>1)
    {
      std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " thick_node->base_name=" << " el->name=" << std::left << std::setw(MaTh::el_type_maxlen) << thick_node->base_name << " i=" << i << " middle=" << middle << " nslices=" << nslices;
    ...
    if (i==middle && !ThickSLice) // create and place new marker with name of thick_elem  in the middle = poition of thick_elem
    {
      ...
      if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " thick_node " << thick_node->name << " nslices=" << nslices << " i=" << i << " middle=" << middle << " place middle marker at=" << thick_node->at_value << " at_expr=" << at_expr
...
void SeqElList::slice_node_default() // slice/translate and add slices to sliced sequence  sliced_seq,  here make always end markers
{
  ...
  for (int i=1; i<=nslices; ++i) // loop to place the nslices in the sequence
  {
    ...
    if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " i=" << i << " new sliced element slice_i " << slice_i->name << std::endl;
    expression* thin_at_expr=nullptr;
    if (fabs(at_shift(nslices,i,local_slice_style))>0.0)
    {
      if( MaTh::iMoreExpressions<1 ) thin_at_expr = compound_expr(at_expr,thick_node->at_value,"+",  nullptr, length*at_shift(nslices,i,local_slice_style) );  // use length and shift values, no expressions
    ...
    if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " thick_node->base_name=" << " el->name=" << std::left << std::setw(MaTh::el_type_maxlen) << thick_node->base_name << " i=" << i << " middle=" << middle << " nslices=" << nslices << " slice_i->name=" << slice_i->name << " at_expr " << my_dump_expression(at_expr) << " l_expr " << my_dump_expression(l_expr) << '\n';
    ...
    if(slice_i) place_node_at(thick_node, sliced_seq, slice_i,thin_at_expr);
  }
...
element* SeqElList::create_sliced_element(const element* thick_elem, int slice_no)
{
  ...
  if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " " << std::left << std::setw(MaTh::el_name_maxlen) << thick_elem->name << " " << std::setw(MaTh::el_type_maxlen) << thick_node->base_name << " slice_no=" << slice_no << " nslices=" << nslices << " is parent=" << (thick_elem==thick_elem->parent) << '\n';
  const command_parameter* length_param  = return_param_recurse("l",thick_elem);
...
{ // final steps in making sliced element, set sliced name, make element from cmd and put to theSliceList
  ...
  if (nslices==1 && slice_no==1) thin_name=thick_elem->name;
  else thin_name = make_thin_name(thick_elem->name,slice_no);
  sliced_elem = make_element(thin_name.c_str(),parent_name.c_str(),cmd,-1);
  if ( MaTh::Verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << std::endl;
...
{
  if ( MaTh::Verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << "  " << std::setw(MaTh::par_name_maxlen) << thick_node->name << " " << std::setw(MaTh::el_type_maxlen) << thick_node->base_name << " thin_node->length=" << thick_node->length << " l=" << el_par_value("l",thick_node->p_elem) << std::endl;
  node* thin_node = nullptr;
  thin_node = clone_node(thick_node, 0);
  if (el_par_value("l",thick_node->p_elem)>zero)
  {
    if ( MaTh::Verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << "  " << std::setw(MaTh::par_name_maxlen) << thick_node->name << " had length, remove" << '\n';
    thin_node->p_elem = create_sliced_element(thick_node->p_elem,1); // keep original length as lrad
  }
  thin_node->length=0;
...
void SeqElList::slice_node() // main steering, decides how to split an individual node and sends it onto the sliced_seq builder
{
  ...
  {
    if(verbose>1) std::cout << " now see what to do with thick_node=" << std::left << std::setw(MaTh::par_name_maxlen) << thick_node->name <<  " depending on its base=" << std::setw(MaTh::par_name_maxlen) << thick_node->base_name << std::right << '\n';
    ...
    {
      if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " zero length place directly copy of element " << thick_node->name << '\n';
    ...
    else if (NameIsInList(thick_node->base_name,element_to_slice_and_tanslate))
    {
      if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " " << thick_node->name <<  " base=" << std::setw(MaTh::par_name_maxlen) << thick_node->base_name << std::right << " slice_node_translate" << '\n';
      slice_node_translate(); // slice/translate and add slices to sliced sequence
    }
    else if(nslices>1)
    {
      if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " nslices=" << nslices << " place copy of thick_node " << thick_node->name << '\n';
  ...
  else if (thick_node->p_sequ) // nested sequence (not flattened), slice and add the subsequence, this case is checked in fivecell.madx
  {
    ...
    if(verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " place the sliced sub-sequence " << thick_node->p_sequ->name << '\n';
...
void SeqElList::place_thick_slice(const element* thick_elem, element* sliced_elem, const int i) // make nodes for the _s, _b  pieces  and place them in the sequence
{
  ...
  expression* l_par_expr=my_get_param_expression(thick_elem, "l"); // with this l_par_expr should not be NULL
  expression* at_expr = clone_expression(thick_node->at_expr);
  double at = thick_node->at_value;
  ...
  place_node_at(thick_node,sliced_seq,sliced_elem,at_expr);
}

void  SeqElList::place_start_or_end_marker(const bool at_start)
{
  const element* thick_elem=thick_node->p_elem;
  if(verbose>1) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << " thick_node " << thick_node->name << " at_start=" << at_start << '\n';
  ...
  element* start_end_marker=new_marker_element(thick_elem->name+AddToName,thick_elem);
  place_thin_slice(thick_node,sliced_seq,start_end_marker,rel_shift);
}
...
sequence* SequenceList::get_sequ(sequence* thick_sequ) // check if thick_sequ is already in my_sequ_list_vec
{
  ...
  for(unsigned int i=0; i<my_sequ_list_vec.size(); ++i)
  {
    ...
    {
      if ( MaTh::Verbose>1) std::cout << __FILE__ << " " << __PRETTY_FUNCTION__ << " line " << std::setw(4) << __LINE__ << " found at i=" << i << '\n'; // debug
      return thick_sequ;
    }
  }
  return nullptr;
...
void SequenceList::put_sequ(sequence* thick_sequ)
{
  ...
  if(MaTh::Verbose>1) std::cout << __FILE__ << " " << __FUNCTION__ << " line " << std::setw(4) << __LINE__ << " my_sequ_list_vec.size()=" << my_sequ_list_vec.size() << '\n';
  return;
}
